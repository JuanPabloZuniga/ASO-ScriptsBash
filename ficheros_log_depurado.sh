...
do
    ...
    nombre="$HOME/"`echo $linea | awk '{print $9}'`
    #echo $nombre
    ...
    if [[ "$mes" == "$mes_hoy" && "$dia" == "$dia_hoy" ]]; then
        ...
        salida="$dia/$mes:$horaMinutoModificado:$propietario:$nombre:"
        ...
        #echo $salida
            ...
            acceso=`stat $nombre | grep -Ei "^Acces.: [0-9]" | cut -f2,3 -d" " | sed "s/\.[0-9]*//"`
            modificacion=`stat $nombre | grep -Ei "^Modi" | cut -f2,3 -d" " | sed "s/\.[0-9]*//"`
            creacion=`stat $nombre | grep -Ei "(Crea)|(Birth)" | tr -s " " | cut -f3,4 -d" " | sed "s/\.[0-9]*//"`
                        
		    if [[ "$creacion" != "$modificacion" || "$creacion" != "$acceso" ]]; then
